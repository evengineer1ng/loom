...
CONCEPT = {
    ...
    "id": "fetch.http_sync.http_json_post",
    "kind": "bridge",
    "version": "0.1.0",
    "deterministic": False,
    ...
    "requires": [],
    "provides": ["fetch.http_json_post"],
    "side_effects": ["network_write", "network_read"],
    "ui_slots": [],
    "tags": ["http", "json", "post"],
    "description": "Perform an HTTP POST with JSON and decode a JSON response body.",
}
...
def api_post(
    ...
    response = client.post(url, headers=headers or {}, json=payload, timeout=timeout)
...
def run(input_packet: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    with httpx.Client() as client:
        value = api_post(client, url, payload=body, headers=headers, timeout=timeout)
    output_packet = {
...
def receipts(output_packet: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "receipt_id": "http-json-post",
            "brick_id": CONCEPT["id"],
            "kind": "network_exchange",
            "label": "Posted JSON and received JSON over HTTP.",
