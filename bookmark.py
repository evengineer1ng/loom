...
# The brick palette — the reskin of the old "Widget:" dropdown. Imported from the sibling
# bookmark/ package; guarded so a missing/broken package never blocks the legacy runtime.
try:
    from bookmark.brick_kernel import BrickRegistry, BRICK_ROOTS, serve_brick
    from bookmark.palette import BrickBar
    from bookmark.draft import DraftGraph
    _HAS_BRICKS = True
except Exception as _brick_import_err:  # pragma: no cover - optional during migration
    _HAS_BRICKS = False
    print(f"[bookmark] brick palette unavailable: {_brick_import_err}")


def _read_global_config() -> dict:
    """Read the shared RadioOS global config (the same file Settings writes), so the courtroom's
    27b endpoint is configured in one place. Returns {} if unavailable."""
    try:
        if os.name == "nt":
            base = os.path.join(os.getenv("APPDATA", os.path.expanduser("~")), "RadioOS")
        else:
            base = os.path.join(os.path.expanduser("~"), ".radioOS")
...
class CourtroomChat(tk.Frame):
    """The single dedicated window: the local OpenCloset Courtroom chat — a brick-laying harness.

    Describe a fuzzy capability; the informant retrieves the bricks that already cover it (so you
    LAY a brick instead of writing code) via deterministic LEXICAL retrieval over the brick
    registry (always works, no model). Then the local 27b (configured in Settings) replies,
    informed by that brief. If the 27b is offline the brief stands on its own.
    """
    BG = "#0b0f17"; FIELD = "#0e1422"; EDGE = "#33507a"; TXT = "#e6e6e6"; MUT = "#8aa0c8"; ACC = "#f4c66b"

    def __init__(self, master, registry, width=360, **kw):
        super().__init__(master, bg=self.BG, width=width,
                         highlightthickness=1, highlightbackground=self.EDGE, **kw)
        self.pack_propagate(False)
        self.registry = registry
        tk.Label(self, text="🎟️ OpenCloset Courtroom", bg=self.BG, fg=self.ACC,
                 font=("Segoe UI", 11, "bold"), anchor="w").pack(fill="x", padx=10, pady=(8, 2))
        tk.Label(self, text="local brick-laying harness — say what you want to build",
                 bg=self.BG, fg=self.MUT, font=("Segoe UI", 8), anchor="w",
                 wraplength=width - 20, justify="left").pack(fill="x", padx=10, pady=(0, 6))
        self.log = tk.Text(self, bg=self.FIELD, fg=self.TXT, relief="flat", wrap="word",
                           font=("Segoe UI", 9), padx=8, pady=8, state="disabled", highlightthickness=0)
        self.log.pack(fill="both", expand=True, padx=8)
        self.log.tag_config("you", foreground=self.ACC)
        self.log.tag_config("court", foreground=self.MUT)
        bar = tk.Frame(self, bg=self.BG)
        bar.pack(fill="x", padx=8, pady=8)
        self.entry = tk.Entry(bar, bg=self.FIELD, fg=self.TXT, insertbackground=self.ACC,
                              relief="flat", font=("Segoe UI", 10))
        self.entry.pack(side="left", fill="x", expand=True, ipady=5)
        self.entry.bind("<Return>", lambda e: self._send())
        tk.Button(bar, text="Send", command=self._send, bg=self.ACC, fg="#241804",
                  relief="flat", font=("Segoe UI", 9, "bold"), padx=12).pack(side="right", padx=(6, 0))
        self._say("court", "Describe a capability — I'll point you at the bricks that already cover it.")

    def _say(self, who, text):
        self.log.config(state="normal")
        prefix = {"you": "you · ", "court": "informant · ", "brief": ""}.get(who, "")
        self.log.insert("end", "\n" + prefix, (who,))
    ...
    def _send(self):
        ...
        if not hits:
            brief_text = "No brick covers that yet — a candidate to author new (missing_brick gap)."
            self._say("court", brief_text)
        else:
            lines = ["Lay these (prefer them over new code):"]
            emojis = []
            for _score, b in hits:
                c = getattr(b, "concept", {}) or {}
                bid = getattr(b, "id", "") or c.get("id", "")
                emoji = getattr(b, "emoji", "") or "•"
                emojis.append(emoji)
                desc = (c.get("description", "") or "")[:80]
                lines.append(f"  {emoji} {bid} — {desc}")
            lines.append("lay-by-emoji: " + " ".join(emojis))
            brief_text = "\n".join(lines)
            self._say("brief", brief_text)
        # Hand off to the local 27b (configured in Settings), informed by the brief.
        cfg = self._llm_config()
        if cfg["enabled"]:
            self._say("court", "asking the local 27b…")
            import threading
            threading.Thread(target=self._llm_turn, args=(q, brief_text), daemon=True).start()

    # ---- the local 27b (OpenAI-compatible, from Settings / global config) ----
    def _llm_config(self):
        cfg = _read_global_config()
        cr = cfg.get("courtroom", {}) if isinstance(cfg.get("courtroom"), dict) else {}
        return {
            "enabled": bool(cr.get("enabled", True)),
            "base_url": (cr.get("base_url") or "http://127.0.0.1:9080").rstrip("/"),
            "model": cr.get("model") or "local-model",
            # Thinking models spend most of the budget on reasoning before the answer — keep it high.
            "max_tokens": int(cr.get("max_tokens", 4096)),
        }

    def _llm_turn(self, prompt, brief_text):
        try:
            reply = self._ask_llm(prompt, brief_text)
            msg = reply or "(empty reply)"
        except Exception as e:
            msg = f"27b offline — the brief stands. ({type(e).__name__})"
        self.after(0, lambda: self._say("court", msg))

    def _ask_llm(self, prompt, brief_text):
        import urllib.request
        cfg = self._llm_config()
        body = json.dumps({
            "model": cfg["model"],
            "messages": [
                {"role": "system", "content": "You are a local coding assistant inside RibbonOS. "
                 "PREFER laying the existing bricks the informant found over writing new code. Be concise."},
                {"role": "user", "content": f"{prompt}\n\n[informant brief]\n{brief_text}"},
            ],
            "max_tokens": cfg["max_tokens"],
            "temperature": 0.4,
        }).encode("utf-8")
        req = urllib.request.Request(cfg["base_url"] + "/v1/chat/completions", data=body,
    ...
    def _retrieve(self, query, k=6):
        import re
        stop = {"the", "a", "an", "to", "of", "for", "and", "or", "is", "it", "that", "this",
                "with", "make", "build", "want", "need", "i", "my", "me", "get", "add", "new", "do", "how"}
        ...
        if not q:
            return []
        scored = []
        bricks = self.registry.bricks
        items = bricks.values() if isinstance(bricks, dict) else bricks
        for b in items:
            c = getattr(b, "concept", {}) or {}
            bid = getattr(b, "id", "") or c.get("id", "")
            hay = " ".join([bid, " ".join(c.get("tags", []) or []),
                            " ".join(c.get("provides", []) or []), c.get("description", "") or ""])
            ht = toks(hay)
            inter = q.intersection(ht)
            if not inter:
                continue
            score = len(inter) + len(q.intersection(set(toks(bid)))) * 1.5
...
class StationUI:
    def __init__(self, widget_registry: WidgetRegistry):
        ...
        self._draft = None               # DraftGraph of laid bricks; built when the registry loads
        ...
        self.root.rowconfigure(0, weight=0)  # brick bar (new top strip)
        self.root.rowconfigure(1, weight=0)  # toolbar
        self.root.rowconfigure(2, weight=1)  # main panels
        self.root.rowconfigure(3, weight=0)  # subtitles
        self.root.columnconfigure(0, weight=1)

        # New top strip for organizing bricks. The original layout (toolbar + 3 windows +
        # subtitles) is unchanged — it just shifts down one row beneath this strip.
        self._build_brick_bar()
        ...
        self.window_canvas.grid(row=2, column=0, sticky="nsew", padx=0, pady=0)
        
        # Window manager state kept (empty) for back-compat with save/load + apply_art.
        self.floating_windows = {}  # {window_id: FloatingWindow}
        self.widget_panels = {}     # {window_id: WidgetPanel}
        # The 3 floating widget-windows are retired (widgets can't be dropped into them anymore).
        # These stay defined as None so the legacy panel loops/apply_art/save-load no-op cleanly.
        self.panel_left = None
        self.panel_center = None
        self.panel_right = None

        # The main area is now a dedicated AUTHORING SPLIT:
        #   left  — the brick CANVAS (snap-column; distinct surface, scroll/pinch zoom, drag pan),
        #           sharing self._draft so a brick laid from the bar appears here immediately.
        #   right — ONE window: the OpenCloset Courtroom chat (the local brick-laying harness).
        self._snap_canvas = None
        self._courtroom = None
        if getattr(self, "brick_registry", None) is not None:
            try:
                from bookmark.canvas import SnapCanvas
                self._courtroom = CourtroomChat(self.window_canvas, self.brick_registry, width=360)
                self._courtroom.pack(side="right", fill="y")
                self._snap_canvas = SnapCanvas(self.window_canvas, self.brick_registry, draft=self._draft)
                self._snap_canvas.pack(side="left", fill="both", expand=True)
                # delete/move happen inside the canvas; mirror the resolution to the debug subtitle.
                def _canvas_changed():
                    try:
                        self.set_subtitle(self._draft.resolve().summary())
                    except Exception:
                        pass
                self._snap_canvas.on_change = _canvas_changed
            except Exception as e:
                print(f"[bookmark] authoring canvas unavailable: {e}")

        # =========================
        # TOOLBAR (Built deferred)
        # =========================
        # Must be built after canvas exists because apply_art/load_layout depend on it
        self._build_toolbar()

        # Legacy builtin widgets are no longer installed — bricks live on the canvas now.
        # (_install_builtin_widgets() guards itself to a no-op when panels are None.)
    ...
    # Toolbar
    # -----------------------
    def _build_brick_bar(self):
        """Build the top brick-bar strip (reskin of the old flat Widget dropdown).

        A thin row-0 strip across the top; the original toolbar/windows/subtitles layout sits
        beneath it unchanged. Degrades to nothing if the bookmark/ package or the bricks root
        is unavailable, so the legacy runtime still boots.
        """
        if not _HAS_BRICKS:
            return
        strip = tk.Frame(self.root, bg="#0e0e0e")
        strip.grid(row=0, column=0, sticky="ew")
        try:
            self.brick_registry = BrickRegistry.from_roots(BRICK_ROOTS)
        except Exception as e:
            print(f"[bookmark] brick bar: registry load failed: {e}")
            return
        self._draft = DraftGraph(self.brick_registry)  # laid bricks resolve here by type
        # Export Oradio sits at the right of the strip; the brick bar fills the rest.
        self.btn_export = tk.Button(
            strip, text="⬇ Export Oradio", font=("Segoe UI", 10, "bold"),
            bg="#1b2733", fg="#e6e6e6", activebackground="#2f4456", activeforeground="#ffffff",
            relief="flat", bd=0, padx=12, pady=4, cursor="hand2",
            command=self._open_export_dialog,
        )
        self.btn_export.pack(side="right", padx=8, pady=4)
        # Settings opens the shared global RadioOS settings (lifted from the shell, same config),
        # so the kernel can change global settings from within.
        self.btn_settings = tk.Button(
            strip, text="⚙ Settings", font=("Segoe UI", 10),
            bg="#1b2733", fg="#e6e6e6", activebackground="#2f4456", activeforeground="#ffffff",
            relief="flat", bd=0, padx=12, pady=4, cursor="hand2",
            command=self._open_global_settings,
        )
        self.btn_settings.pack(side="right", padx=(0, 4), pady=4)
        # The Club is itself a brick (the highest-level one) — open it via the brick system, not a
        # hardcoded panel. Ships in the kernel; this is the convenience entry to it.
        self.btn_club = tk.Button(
            strip, text="◎ Club", font=("Segoe UI", 10),
            bg="#1b2733", fg="#e6e6e6", activebackground="#2f4456", activeforeground="#ffffff",
            relief="flat", bd=0, padx=12, pady=4, cursor="hand2",
            command=self._open_club,
        )
        self.btn_club.pack(side="right", padx=(0, 4), pady=4)
        self.brick_bar = BrickBar(strip, self.brick_registry, on_place=self._on_place_brick)
        self.brick_bar.pack(side="left", fill="x", expand=True)
        print(f"[bookmark] brick bar: {len(self.brick_registry.available())} bricks ready")

    def _on_place_brick(self, brick):
        """A brick was laid from the bar — add it to the draft and re-resolve by type.

        The stack canvas isn't built yet, so feedback lands in the subtitle bar (now repurposed
        for sim/debug): how many bonds formed, holes opened (Club's job), and strata deep.
        """
        if self._draft is None:
            return
        # Place via the canvas when present (places on the shared draft AND renders the block),
        # so a laid brick is immediately visible; else fall back to the bare draft.
        if getattr(self, "_snap_canvas", None) is not None:
            inst = self._snap_canvas.place(brick)
        else:
            inst = self._draft.place(brick)
        res = self._draft.resolve()
        try:
            self.set_subtitle(f"laid {inst.instance_id}    ·    {res.summary()}")
        except Exception:
            pass
        print(f"[bookmark] laid {inst.instance_id}  | {res.summary()}")

    def _open_club(self):
        """Open the Club — a surface brick (club.surface.manager) served locally."""
        try:
            brick = self.brick_registry.get("club.surface.manager")
            if brick is None:
                self.set_subtitle("Club brick not found in registry")
                return
            try:
                from bookmark.club_manager import open_club_manager

                res = open_club_manager(brick.asset)
            except Exception:
                res = serve_brick(brick)
            if res.get("error"):
                self.set_subtitle(f"Club: {res['error']}")
            else:
                self.set_subtitle(f"Club open → {res.get('url')}")
        except Exception as e:
            print(f"[bookmark] club open failed: {e}")

    def _open_global_settings(self):
        """Launch the shared RadioOS settings window (ribbon_os_shell --settings). Same global
        config file, so changes here affect RibbonOS globally."""
        import os
        import subprocess
        import sys
        here = os.path.dirname(os.path.abspath(__file__))
        shell = os.path.join(here, "ribbon_os_shell.py")
        try:
            subprocess.Popen([sys.executable, shell, "--settings"], cwd=here)
        except Exception as e:
            try:
                self.set_subtitle(f"settings launch failed: {e}")
            except Exception:
                print(f"[bookmark] settings launch failed: {e}")

    # ---- Export / mint an .oradio -------------------------------------------------
    def _draft_brick_ids(self):
        """The brick ids currently laid in the draft — becomes the oradio's `bricks` field."""
        try:
            return [p.brick.id for p in (self._draft.placed if self._draft else [])]
        except Exception:
            return []

    def _club_dir(self):
        import os
        return os.path.join(os.path.dirname(os.path.abspath(__file__)), "club")

    def _ensure_author_identity(self, parent):
        """Return the current author's provenance stamp, raising the one-time questionnaire first
        if no identity exists yet. Returns None only if the user cancels the questionnaire."""
        try:
            from bookmark import identity
        except Exception:
            return {}   # identity module missing -> don't block minting
        club = self._club_dir()
        stamp = identity.author_stamp(club)
        if stamp:
            return stamp
        return self._prompt_identity_dialog(parent)

    def _prompt_identity_dialog(self, parent):
        """The one-time, 3-question provenance questionnaire. Saves locally + returns the stamp
        ({name, seed, identity_version}), or None if cancelled. See bookmark/identity.py."""
        from bookmark import identity
        dlg = tk.Toplevel(parent)
        dlg.title("Who's authoring this? (one-time)")
        dlg.configure(bg="#0e0e0e")
        dlg.geometry("560x430")
        dlg.transient(parent)
        dlg.grab_set()

        tk.Label(dlg, text="✶ Provenance", bg="#0e0e0e", fg="#f4c66b",
                 font=("Segoe UI", 12, "bold")).pack(anchor="w", padx=16, pady=(16, 2))
        tk.Label(dlg, text="Asked once. Your answers never leave this machine — only a derived\n"
                           "seed + your name are stamped into what you author, so your lineage\n"
                           "is traceable and two people with the same name still differ.",
                 bg="#0e0e0e", fg="#8a8a8a", font=("Segoe UI", 9), justify="left").pack(
            anchor="w", padx=16, pady=(0, 8))

        questions = [
            ("What's your name — how do you identify yourself?", "e.g. evengineer1ng"),
        ...
        for q, hint in questions:
            tk.Label(dlg, text=q, bg="#0e0e0e", fg="#e6e6e6",
                     font=("Segoe UI", 10)).pack(anchor="w", padx=16, pady=(8, 0))
            e = tk.Entry(dlg, bg="#1b2733", fg="#e6e6e6", insertbackground="#e6e6e6",
                         relief="flat", font=("Segoe UI", 10))
            e.pack(fill="x", padx=16)
            tk.Label(dlg, text=hint, bg="#0e0e0e", fg="#5d6b7a",
                     font=("Segoe UI", 8)).pack(anchor="w", padx=16)
            entries.append(e)
        entries[0].focus_set()

        seed_lbl = tk.Label(dlg, text="seed: —", bg="#0e0e0e", fg="#6f8cff",
                            font=("Consolas", 9))
        seed_lbl.pack(anchor="w", padx=16, pady=(10, 0))
        ...
        def _save():
            ...
            if not name:
                ...
                return
            ident = identity.set_identity(self._club_dir(), name,
                                          entries[1].get(), entries[2].get())
            result["stamp"] = ident.stamp()
            dlg.destroy()

        btns = tk.Frame(dlg, bg="#0e0e0e")
        btns.pack(fill="x", padx=16, pady=14, side="bottom")
        tk.Button(btns, text="Seal identity", command=_save, bg="#3a6b4a", fg="#ffffff",
                  relief="flat", bd=0, padx=14, pady=6, font=("Segoe UI", 10, "bold"),
                  cursor="hand2").pack(side="right")
        tk.Button(btns, text="Cancel", command=dlg.destroy, bg="#1b2733", fg="#e6e6e6",
    ...
    def _open_export_dialog(self):
        """Gather mint inputs and export a .oradio via bookmark.mint.mint_oradio.

        The kernel checkbox is the soulmate exception: a kernel may stand alone; everything else
        needs a soulmate (oradio id + which loom). Bricks are taken from the current draft.
        """
        from pathlib import Path

        from oradio_engine.loom_graph import load_declaration_text, slugify_node
        from oradio_engine.loom_runtime import discover_all_looms, resolve_active_loom

        win = tk.Toplevel(self.root)
        win.title("Export .oradio")
        ...
        pad = {"padx": 10, "pady": 4}
        def label(text, r):
            tk.Label(win, text=text, bg="#0e0e0e", fg="#8a8a8a",
                     font=("Segoe UI", 9)).grid(row=r, column=0, sticky="w", **pad)
        def entry(r, default=""):
            e = tk.Entry(win, bg="#1b2733", fg="#e6e6e6", insertbackground="#e6e6e6",
                         relief="flat", font=("Segoe UI", 10))
            if default:
                e.insert(0, default)
        ...
        # video picker
        label("Loop video (2.5-5s mp4/ogv)", 0)
        vid_var = tk.StringVar()
        vrow = tk.Frame(win, bg="#0e0e0e")
        vrow.grid(row=0, column=1, sticky="ew", **pad)
        vrow.columnconfigure(0, weight=1)
        vid_entry = tk.Entry(vrow, textvariable=vid_var, bg="#1b2733", fg="#e6e6e6",
                             insertbackground="#e6e6e6", relief="flat", font=("Segoe UI", 10))
        vid_entry.grid(row=0, column=0, sticky="ew")
        def browse():
            p = filedialog.askopenfilename(
                title="Choose loop video",
                filetypes=[("Video", "*.mp4 *.ogv *.mov *.webm *.mkv"), ("All", "*.*")],
            )
            if p:
                vid_var.set(p)
        tk.Button(vrow, text="…", command=browse, bg="#1b2733", fg="#e6e6e6",
                  relief="flat", cursor="hand2").grid(row=0, column=1, padx=(6, 0))

        label("Oradio id", 1);          id_e = entry(1)
        label("Title", 2);              title_e = entry(2)
        label("Declaration", 3);        decl_e = entry(3)

        kernel_var = tk.BooleanVar(value=False)
        project_root = Path(os.path.dirname(os.path.abspath(__file__)))
        known_looms = discover_all_looms(project_root)
        loom_options: List[str] = []
        loom_lookup: Dict[str, tuple] = {}
        for loom_path, loom_id, label_text in known_looms:
            option = f"{loom_id}  ·  {label_text}" if label_text != loom_id else loom_id
            loom_options.append(option)
            loom_lookup[option] = (loom_path, loom_id, label_text)
        active_loom = resolve_active_loom(project_root)
        default_loom = next((opt for opt, (path, _lid, _lbl) in loom_lookup.items()
                             if active_loom is not None and path.resolve() == active_loom.resolve()), "")
        loom_var = tk.StringVar(value=default_loom or (loom_options[0] if loom_options else ""))
        soul_var = tk.StringVar(value="")
        soul_id_e = ttk.Combobox(win, textvariable=soul_var, values=[], state="readonly")
        soul_id_e.grid(row=5, column=1, sticky="ew", **pad)
        loom_e = ttk.Combobox(win, textvariable=loom_var, values=loom_options, state="readonly")
        loom_e.grid(row=6, column=1, sticky="ew", **pad)
        def toggle_kernel(*_):
            state = "disabled" if kernel_var.get() else "normal"
            soul_id_e.config(state=state); loom_e.config(state=state)
        tk.Checkbutton(
            win, text="kernel (soulmate exception — can stand alone)", variable=kernel_var,
            command=toggle_kernel, bg="#0e0e0e", fg="#e6e6e6", selectcolor="#1b2733",
            activebackground="#0e0e0e", activeforeground="#e6e6e6", font=("Segoe UI", 9),
        ).grid(row=4, column=0, columnspan=2, sticky="w", **pad)
        label("Soulmate oradio id", 5)
        label("Mint into loom", 6)

        loom_hint = tk.Label(win, text="", bg="#0e0e0e", fg="#8a8a8a", font=("Segoe UI", 8),
                             wraplength=420, justify="left", anchor="w")
        loom_hint.grid(row=6, column=1, sticky="sw", padx=10, pady=(30, 4))

        def selected_loom_info():
            return loom_lookup.get(loom_var.get().strip())

        def refresh_loom_choices(*_):
            info = selected_loom_info()
            values: List[str] = []
            if info is not None:
                loom_path, loom_id, _label_text = info
                try:
                    universe, nodes = load_declaration_text(loom_path.read_text(encoding="utf-8"))
                except Exception:
                    universe, nodes = "", []
                current_id = slugify_node(id_e.get().strip() or title_e.get().strip())
                values = [str(node.get("id", "")).strip() for node in nodes
                          if str(node.get("id", "")).strip() and str(node.get("id", "")).strip() != current_id]
                loom_hint.config(text=f"current loom: {loom_id} · {len(values)} candidate soulmate ids")
            else:
                loom_hint.config(text="no known loom selected")
            soul_id_e["values"] = values
            if values:
                if soul_var.get().strip() not in values:
                    soul_var.set(values[0])
            else:
                soul_var.set("")

        loom_e.bind("<<ComboboxSelected>>", refresh_loom_choices)
        id_e.bind("<KeyRelease>", refresh_loom_choices)
        title_e.bind("<KeyRelease>", refresh_loom_choices)
        refresh_loom_choices()

        brick_ids = self._draft_brick_ids()
        label("Bricks (from draft)", 7)
        tk.Label(win, text=(", ".join(brick_ids) or "(none laid)"), bg="#0e0e0e",
                 fg="#8a8a8a", font=("Segoe UI", 9), wraplength=320, justify="left",
                 anchor="w").grid(row=7, column=1, sticky="w", **pad)

        # Loop mode: ping-pong (force any clip to loop) vs as-is (the clip ALREADY loops —
        # ribbons, a clean corner exit — don't mangle it). See bookmark/mint.bake_loop.
        loop_mode_var = tk.StringVar(value="pingpong")
        lm = tk.Frame(win, bg="#0e0e0e")
        lm.grid(row=8, column=0, sticky="w", **pad)
        tk.Label(lm, text="Loop:", bg="#0e0e0e", fg="#8a8a8a",
                 font=("Segoe UI", 9)).pack(side="left")
        for val, txt in (("pingpong", "ping-pong"), ("as_is", "as-is (already loops)")):
            tk.Radiobutton(lm, text=txt, value=val, variable=loop_mode_var,
                           bg="#0e0e0e", fg="#e6e6e6", selectcolor="#1b2733",
                           activebackground="#0e0e0e", activeforeground="#e6e6e6",
                           font=("Segoe UI", 9)).pack(side="left", padx=(4, 0))

        status = tk.Label(win, text="", bg="#0e0e0e", fg="#8a8a8a", font=("Segoe UI", 9),
                          wraplength=480, justify="left")
        status.grid(row=9, column=0, columnspan=2, sticky="we", **pad)

        def do_mint():
            video = vid_var.get().strip()
            oid = id_e.get().strip()
            if not video or not oid:
                status.config(text="need a loop video and an oradio id", fg="#b85c5c")
                return
            is_kernel = kernel_var.get()
            soulmates = None
            target_loom_path = None
            target_soulmates: List[str] = []
            if not is_kernel:
                info = selected_loom_info()
                sid = soul_var.get().strip()
                if info is None:
                    status.config(text="choose an existing loom for this mint",
                                  fg="#b85c5c")
                    return
                target_loom_path, loom, _label_text = info
                if not sid:
                    status.config(text="non-kernel needs a real soulmate id from that loom",
                                  fg="#b85c5c")
                    return
                soulmates = {loom: [sid]}
                target_soulmates = [sid]
            # Provenance gate: the one-time author questionnaire fires here on a first mint.
            author = self._ensure_author_identity(win)
            if author is None:
                status.config(text="mint needs an author identity (provenance) — cancelled",
                              fg="#b85c5c")
                return
            status.config(text=f"minting… (by {author.get('name','?')})", fg="#e6e6e6")
            mint_btn.config(state="disabled")
            self._run_mint(win, status, mint_btn, video, oid,
                           title_e.get().strip() or oid, decl_e.get().strip(),
                           soulmates, brick_ids, is_kernel, author, loop_mode_var.get(),
                           target_loom_path=target_loom_path, target_soulmates=target_soulmates)

        mint_btn = tk.Button(win, text="Mint .oradio", command=do_mint,
                             bg="#3a6b4a", fg="#ffffff", relief="flat", bd=0, padx=14, pady=6,
                             font=("Segoe UI", 10, "bold"), cursor="hand2")
        mint_btn.grid(row=8, column=1, sticky="e", **pad)

    def _run_mint(self, win, status, mint_btn, video, oid, title, decl, soulmates, bricks,
                  is_kernel, author=None, loop_mode="pingpong", target_loom_path=None,
                  target_soulmates=None):
        import os
        import threading
        from pathlib import Path
        ...
        def worker():
            try:
                from bookmark.mint import mint_oradio
                from oradio_engine.loom_runtime import request_ribbonos_load, sync_crossovers, upsert_oradio_into_loom

                path = mint_oradio(
                    video, oradio_id=oid, title=title, out_dir=out_dir,
                    declaration=decl, soulmates=soulmates, bricks=bricks, is_kernel=is_kernel,
                    author=author, loop_mode=loop_mode,
                )
                loom_note = ""
                if target_loom_path is not None:
                    try:
                        update = upsert_oradio_into_loom(
                            Path(target_loom_path),
                            Path(path),
                            soulmate_ids=list(target_soulmates or []),
                            label=title,
                        )
                        built, failed = sync_crossovers(Path(os.path.dirname(os.path.abspath(__file__))),
                                                        Path(target_loom_path),
                                                        update["universe"],
                                                        update["nodes"])
                        request_ribbonos_load(Path(os.path.dirname(os.path.abspath(__file__))), Path(target_loom_path))
                        loom_note = f" · loaded into loom ({built} crossovers"
                        if failed:
                            loom_note += f", {failed} skipped"
                        loom_note += ")"
                    except Exception as loom_exc:
                        loom_note = f" · loom sync skipped: {loom_exc}"
                self.root.after(0, lambda: done(str(path), None, loom_note))
            except Exception as e:
                self.root.after(0, lambda: done(None, str(e), ""))

        def done(path, err, loom_note):
            try:
                mint_btn.config(state="normal")
            except Exception:
                pass
            if err:
                status.config(text=f"mint failed: {err}", fg="#b85c5c")
                return
            status.config(text=f"minted → {path}{loom_note}", fg="#4cf0a1")
            try:
                self.set_subtitle(f"minted .oradio → {path}")
            except Exception:
                pass
    ...
    def _build_toolbar(self):
        ...
        self._toolbar_bg_label.grid(row=1, column=0, sticky="ew", padx=0, pady=0)
        self._toolbar_bg_label.lower()

        bar = tk.Frame(self.root, bg=toolbar_bg)
        self._toolbar_frame = bar
        bar.grid(row=1, column=0, sticky="ew", padx=8, pady=(8,0))
        ...
        # Panel/Widget dropdowns retired — the top brick bar is now how you find and place
        # bricks. (The floating-window widget flow itself retires with the canvas.)
        ...
        # (Add Widget retired — place bricks from the top brick bar)
        
        # (Remove Current retired)
    ...
    def _install_builtin_widgets(self):
        # The legacy widget-panels are retired (bricks live on the canvas now). No panels -> no-op.
        if getattr(self, "panel_left", None) is None:
            return
        ...
        # (toolbar widget dropdown retired; nothing to seed)
...
def main():
    ...
    try:
        ...
        # Authoring/reskin: Bookmark must open even with no station configured. A missing
        # meta plugin is no longer fatal — the UI (and brick palette) come up regardless.
        ACTIVE_META_PLUGIN = None
        runtime_context["ACTIVE_META_PLUGIN"] = None
        shared_runtime["ACTIVE_META_PLUGIN"] = None
        log("meta", "Continuing with no active meta plugin (authoring mode).")
